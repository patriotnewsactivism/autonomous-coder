...
// server/providers.ts
function isProviderActive(name) {
...
function calcCost(model, promptTokens, completionTokens) {
  ...
  const m = provider?.models.find((m2) => m2.id === model);
  const pricing = m?.pricing || [0.15, 0.6];
  return promptTokens / 1e6 * pricing[0] + completionTokens / 1e6 * pricing[1];
}
function buildOpenAIRequest(provider, model, systemPrompt, userMessage, maxTokens, stream) {
  return {
...
function buildCohereRequest(provider, model, systemPrompt, userMessage, maxTokens, stream) {
  return {
    url: provider.endpoint,
    headers: {
      Authorization: `Bearer ${provider.apiKey}`,
...
function buildRequest(model, systemPrompt, userMessage, maxTokens = 4096, stream = false) {
  const provider = findProviderForModel(model);
...
function parseStreamChunk(providerName, line) {
  ...
  "server/providers.ts"() {
    PROVIDERS = {
      deepseek: {
        ...
        models: [
          { id: "deepseek-chat", label: "DeepSeek Chat", contextWindow: 64e3, pricing: [0.14, 0.28] },
          { id: "deepseek-reasoner", label: "DeepSeek Reasoner", contextWindow: 64e3, pricing: [0.55, 2.19] }
        ],
        isFree: false
      },
      groq: {
        ...
        models: [
          { id: "llama-3.3-70b-versatile", label: "Llama 3.3 70B", contextWindow: 128e3, pricing: [0.59, 0.79] },
          { id: "llama-3.1-8b-instant", label: "Llama 3.1 8B Instant", contextWindow: 128e3, pricing: [0.05, 0.08] },
          { id: "qwen-2.5-coder-32b", label: "Qwen 2.5 Coder 32B", contextWindow: 128e3, pricing: [0.59, 0.79] },
      ...
      cerebras: {
        ...
        models: [
          { id: "llama-3.3-70b", label: "Llama 3.3 70B (Cerebras)", contextWindow: 128e3, pricing: [0, 0] },
      ...
      github: {
        ...
        models: [
          { id: "gpt-4o", label: "GPT-4o (GitHub)", contextWindow: 128e3, pricing: [0, 0] },
          { id: "gpt-4o-mini", label: "GPT-4o Mini (GitHub)", contextWindow: 128e3, pricing: [0, 0] },
          { id: "Phi-4", label: "Phi-4 (GitHub)", contextWindow: 16e3, pricing: [0, 0] },
          { id: "DeepSeek-R1", label: "DeepSeek-R1 (GitHub)", contextWindow: 64e3, pricing: [0, 0] }
        ],
        isFree: true
      },
      cohere: {
        ...
        models: [
          { id: "command-r-plus", label: "Command R+ (Cohere)", contextWindow: 128e3, pricing: [0, 0] },
          { id: "command-a", label: "Command A (Cohere)", contextWindow: 256e3, pricing: [0, 0] }
        ],
        isFree: true
      }
    };
...
// server/routes.ts
function calcCost2(model, promptTokens, completionTokens) {
  return calcCost(model, promptTokens, completionTokens);
}
async function callAI(systemPrompt, userMessage, model) {
  ...
  return {
    ...
    model: deployment,
    costUsd: calcCost2(deployment, parsed.promptTokens, parsed.completionTokens)
  };
}
async function callAIStream(systemPrompt, userMessage, onToken, model) {
  ...
  return {
    ...
    completionTokens,
    model: deployment,
    costUsd: calcCost2(deployment, promptTokens, completionTokens)
  };
}
...
${JSON.stringify(prInfo, null, 2)}`
  ...
  app2.post("/api/github/import", async (req, res) => {
    try {
      ...
      const fileResults = await Promise.allSettled(
        filesToFetch.map(async (f) => {
          ...
          if (fileData.size > 1e5) return null;
          const fileContent = Buffer.from(fileData.content, "base64").toString("utf-8");
          return { path: f.path, content: fileContent, size: fileData.size };
        })
      );
      const files = fileResults.filter((r) => r.status === "fulfilled" && r.value !== null).map((r) => r.value);
      res.json({
        fullName,
        name: repo,
...
var systemPromptAnalyze, systemPrompts, DEFAULT_MODEL;
...
// server/superagent.ts
var superagent_exports = {};
__export(superagent_exports, {
...
async function executeSimple(goal, classification, model, onToken) {
  ...
  if (onToken) {
    const { content: content2 } = await callAIStream(prompt, goal, onToken, model);
    return { output: content2, type: "answer" };
  }
...
- For epic tasks, populate subtasks with independently-executable chunks`;
    GENERALIST_PROMPT = `You are an elite Superagent \u2014 highly capable, precise, and autonomous.
...
function registerParallelRoutes(app2) {
  app2.post("/api/superagent/task", async (req, res) => {
    try {
      const { goal, model, sessionId } = req.body;
      if (!goal) return res.status(400).json({ error: "goal required" });
      const { executeTask: executeTask2 } = await Promise.resolve().then(() => (init_superagent(), superagent_exports));
  ...
  app2.get("/api/superagent/classify", async (req, res) => {
    try {
      const { goal, model } = req.query;
      if (!goal) return res.status(400).json({ error: "goal required" });
      const { classifyTask: classifyTask2 } = await Promise.resolve().then(() => (init_superagent(), superagent_exports));
  ...
  app2.get("/api/agent/stream/:sessionId", (req, res) => {
    ...
    const events = [
      ...
      "sandbox:update",
      "superagent:progress",
      "superagent:classified",
      "superagent:classifying",
      "superagent:epic:start",
      "superagent:epic:done",
      "superagent:done"
    ];
    const handlers = {};
